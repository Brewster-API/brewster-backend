...
export default class Drink {
  ...
  static async update(id, userId, { drinkName, brew, description, ingredients, postId }) {
    ...
    const { rows } = await pool.query(
      'UPDATE drinks SET drink_name=$1, brew=$2, drink_description=$3, ingredients=$4  WHERE drinks.id=$5  RETURNING *',
      [newDrinkName, newBrew, newDescription, newIngredients, id]
    );
    return new Drink(rows[0]);
  }
}
