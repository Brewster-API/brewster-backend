...
export default class Drink {
  ...
  static async update(id, { drinkName, brew, description, ingredients, postId }) {
    ...
    const newUserId = postId ?? existingDrink.postId;
    console.log('asdasda', existingDrink);
    const { rows } = await pool.query(
      'UPDATE drinks SET drink_name=$1, brew=$2, drink_description=$3, ingredients=$4, post_id=$5 FROM drinks AS d JOIN users ON users.id = d.post_id  WHERE d.id=$6 RETURNING *',
      [newDrinkName, newBrew, newDescription, newIngredients, newUserId, id]
    );
